...
FOR EACH ROW EXECUTE FUNCTION reject_attempt_backfill()`); err != nil {
	...
	if latestVersion != 23 || migrationCount != 23 {
		t.Errorf("migration history after retry = max %d, count %d; want max 23, count 23", latestVersion, migrationCount)
	}
...
func TestRunSerializesConcurrentCallsAndReleasesLock(t *testing.T) {
	...
	if migrationCount != 23 {
		t.Errorf("migration history count = %d, want 23", migrationCount)
	}
